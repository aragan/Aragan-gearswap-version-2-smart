...
----------------------------------------------------------
-- Weapon type color helper (Relic / Mythic / Aeonic / Empyrean / Prime)
-- Uses simple manual tables keyed by English item names.
----------------------------------------------------------
local manual_relic_weapons = {
    ['Spharai']=true, ['Mandau']=true, ['Excalibur']=true, ['Ragnarok']=true, ['Guttler']=true,
    ['Bravura']=true, ['Apocalypse']=true, ['Gungnir']=true, ['Kikoku']=true, ['Amanomurakumo']=true,
    ['Mjollnir']=true, ['Claustrum']=true, ['Yoichinoyumi']=true, ['Annihilator']=true, ['Gjallarhorn']=true,
    ['Aegis']=true,
}

local manual_mythic_weapons = {
    ['Conqueror']=true, ['Glanzfaust']=true, ['Kenkonken']=true, ['Yagrush']=true, ['Laevateinn']=true,
    ['Murgleis']=true, ['Vajra']=true, ['Burtgang']=true, ['Liberator']=true, ['Aymur']=true,
    ['Carnwenhan']=true, ['Terpsichore']=true, ['Tizona']=true, ['Ryunohige']=true, ['Nagi']=true,
    ['Kogarasumaru']=true, ['Nirvana']=true, ['Tupsimati']=true, ['Gastraphetes']=true, ['Death Penalty']=true,
    ['Idris']=true, ['Epeolatry']=true,
}

local manual_aeonic_weapons = {
    ['Godhands']=true, ['Aeneas']=true, ['Sequence']=true, ['Lionheart']=true, ['Chango']=true,
    ['Tri-edge']=true, ['Anguta']=true, ['Trishula']=true, ['Heishi Shorinken']=true, ['Dojikiri Yasutsuna']=true,
    ['Khatvanga']=true, ['Tishtrya']=true, ['Fomalhaut']=true, ['Fail-Not']=true, ['Marsyas']=true,
    ['Srivatsa']=true,
}

local manual_empyrean_weapons = {
    ['Verethragna']=true, ['Twashtar']=true, ['Almace']=true, ['Caladbolg']=true, ['Farsha']=true,
    ['Ukonvasara']=true, ['Redemption']=true, ['Rhongomiant']=true, ['Kannagi']=true, ['Masamune']=true,
    ['Gambanteinn']=true, ['Hvergelmir']=true, ['Gandiva']=true, ['Armageddon']=true, ['Daurdabla']=true,
    ['Ochain']=true,
}

local manual_prime_weapons = {
    -- Stage 1
    ['Prime Fists']=true, ['Prime Dagger']=true, ['Prime Sword']=true, ['Prime Blade']=true, ['Prime Pickaxe']=true,
    ['Prime Great Axe']=true, ['Prime Scythe']=true, ['Prime Lance']=true, ['Genshitanto']=true, ['Genshito']=true,
    ['Prime Maul']=true, ['Prime Staff']=true, ['Prime Bow']=true, ['Prime Gun']=true, ['Prime Horn']=true,
    ['Prime Shield']=true,
    -- Stage 2Ã¢â‚¬â€œ5
    ['Varga Purnikawa']=true, ['Mpu Gandring']=true, ['Caliburnus']=true, ['Helheim']=true, ['Spalirisos']=true,
    ['Laphria']=true, ['Foenaria']=true, ['Gae Buide']=true, ['Dokoku']=true, ['Kusanagi']=true,
    ['Lorg Mor']=true, ['Opashoro']=true, ['Pinaka']=true, ['Earp']=true, ['Loughnashade']=true,
    ['Duban']=true,
}


----------------------------------------------------------
-- Return color tag and normalized weapon name for HUD
-- Relic    = light blue
-- Mythic   = darker blue
-- Aeonic   = green
-- Empyrean = red
-- Prime    = purple
----------------------------------------------------------
local function get_main_weapon_color_and_name(weapon_name)
    local main = weapon_name or (player and player.equipment and player.equipment.main or nil)

    if not main or main == '' or main == 'empty' then
        return '\\cs(180,180,180)', 'Empty'
    end

    local wname = main

    -- If GearSwap stores an item id instead of a name, try to resolve it
    if type(main) == 'number' and res and res.items and res.items[main] then
        wname = res.items[main].en
    end

    wname = tostring(wname)
    wname = wname:gsub('^%s+', ''):gsub('%s+$', '') -- trim spaces

    local color = '\\cs(255,255,255)'

    local function norm_weapon(s)
    ...
    local wn_norm = norm_weapon(wname)

    local function tbl_contains_weapon(tbl)
        if not tbl or wn_norm == '' then return false end
        for k,_ in pairs(tbl) do
            if wn_norm:find(norm_weapon(k), 1, true) then
                return true
            end
        end
        return false
    ...
    if manual_prime_weapons[wname] or tbl_contains_weapon(manual_prime_weapons) then
        color = '\\cs(200,120,255)' -- purple
    elseif manual_empyrean_weapons[wname] or tbl_contains_weapon(manual_empyrean_weapons) then
        color = '\\cs(220,80,80)'   -- red
    elseif manual_aeonic_weapons[wname] or tbl_contains_weapon(manual_aeonic_weapons) then
        color = '\\cs(60,220,120)'  -- green
    elseif manual_mythic_weapons[wname] or tbl_contains_weapon(manual_mythic_weapons) then
        color = '\\cs(120,200,255)' -- light blue
    elseif manual_relic_weapons[wname] or tbl_contains_weapon(manual_relic_weapons) then
        color = '\\cs(255,255,0)' -- yellow
    end
...
function init_job_states(job_bools,job_modes)
    ...
    -- Copy job_modes into stateList but skip 'Weapons' to avoid duplicate Weapon display
    stateList = {}
    for i,n in ipairs(job_modes) do
        if n ~= 'Weapons' then
            table.insert(stateList, n)
        end
    end
...
function update_job_states()
    ...
    stateBox:clear()
	stateBox:append('   ')
    -- Show Weapon based on selected state.Weapons option (including None). Hide Empty.
    do
        local selected = (state and state.Weapons and state.Weapons.value) or nil
        local w_color, w_name

        if selected and selected ~= '' then
            if selected == 'None' then
                w_color, w_name = '\\cs(180,180,180)', 'None'
            else
                w_color, w_name = get_main_weapon_color_and_name(selected)
            end
        else
            w_color, w_name = get_main_weapon_color_and_name()
        end

        if w_name and w_name ~= 'Empty' then
            stateBox:append(string.format("%sWeapons: %s%s%s", clr.w, w_color or clr.n, w_name, clr.n))
            stateBox:append(spc)
        end
    end
...
----------------------------------------------------------
-- Auto-refresh weapon color on main-hand change
-- This keeps Weapon color updated without needing 'gs c update'.
-- Very lightweight: it only calls update_job_states() if the main weapon actually changed.
----------------------------------------------------------
local last_display_main_weapon = nil
local last_update_time = 0
local update_delay = 0.5 -- Delay in seconds

windower.register_event('prerender', function()
	if not state or not state.DisplayMode or not state.DisplayMode.value then
		return
	end
	...
	local cur_main = player.equipment.main or 'empty'
	local current_time = os.clock()

	if cur_main ~= last_display_main_weapon and (current_time - last_update_time) >= update_delay then
		last_display_main_weapon = cur_main
