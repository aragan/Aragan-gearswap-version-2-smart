...
function init_include()
	...
	state.CraftingMode		  = M{['description'] = 'Crafting Mode','None','Cooking','Fishing','Goldsmithing','Smithing','Woodworking'} --'Leathercraft','Alchemy','Bonecraft','Clothcraft','Synergy'
    ...
    -- Load Ara-Libs first, followed by User-Globals, followed by <character>-Globals.
    ...
    include('Ara-SelfCommands')
	include('Ara-TreasureHunter')
                ...
                if p['Target Name'] == 'Aragan' then
					for i in pairs(naughty_list) do 
						if p['Message']:contains(naughty_list[i]) then
							...
							windower.add_to_chat(123,'Contact me on Discord: aragan1253 or https://discord.gg/xjE99FWGRQ')
							return true
						end
					end
                end
...
-- Global  - user_xxx() functions get called first.  Define in Ara-Globals or User-Globals.
...
function equip_gear_by_status(playerStatus, petStatus)
	if _global and _global.debug_mode then add_to_chat(123,'Debug: Equip gear for status ['..tostring(playerStatus)..'], HP='..tostring(player.hp)) end
	if player.hp > 0 then
...
function get_idle_set(petStatus)
	...
	if not (player.in_combat or being_attacked) and 
	(state.IdleMode.current:contains('Tank') or (state.IdleMode.current:contains('DT')
			and not state.IdleMode.current:contains('MDT')
			and not state.IdleMode.current:contains('PDT'))) then
    ...
    if data.areas.cities:contains(world.area) then
		...
		if not (state.CraftingMode.value or state.CraftQuality.value) and (world.area:contains('Adoulin') or world.area == "Celennia Memorial Library") and item_available("Councilor's Garb") then
...
function get_resting_set()
	...
	if state.NakedMode.value then return end
